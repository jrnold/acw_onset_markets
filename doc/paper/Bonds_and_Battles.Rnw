...
The spread jumped approximately 0.25 percentage points in November 1856.
However, the reasons for this jump are unclear.
The timing corresponds with the election of James Buchanan.
...
From July 1857 to January 1860, the spread had mean of \Sexpr{round(SPREAD_1857_1860, 2)} and was stable;
the mean difference between the spread in January 1860 and July 1857 was \Sexpr{round(SPREAD_DIFF_1860_1857_MEAN, 2)} ($\Pr(x > 0) = \Sexpr{round(SPREAD_DIFF_1860_1857_PVAL, 3)}$).

The spread trended slightly upward during 1860, reaching \Sexpr{round(SPREAD_18601102, 2)} on November 2, 1860.
The difference between the spread on November 1860 and January 1860 had a mean \Sexpr{round(SPREAD_DIFF_1860_MEAN, 2)} ($\Pr(x > 0) = 1$).
The spread jumps at and after the election of Lincoln;
increasing \Sexpr{round(SPREAD_DIFF_ELEC, 2)} percentage points from November 2--23, and another \Sexpr{round(SPREAD_DIFF_POSTELEC, 2)} from November 23 to April 13, 1861, reaching a value of \Sexpr{round(SPREAD_18610413, 2)}.
...
There were no changes in war risk between 1857 and 1860.
There are small increase in war risk the election of 1860.
...
However, the spread is much lower than would be observed after Fort Sumter, implying that the start of the war was largely unexpected.
...
Since in this case the war did occur, $R$ is known --- it is the price of the bond observed at the start of the war.
...
However, better estimates of the probabaility of war can be made by noting that the probability of war is common across all bonds.
Thus the probability of war can be estimated as,
\begin{align*}
