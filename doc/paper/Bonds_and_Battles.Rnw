...
An approximation of the default risk based on bond yields \parencite{Hull2004},
\begin{equation}
  \label{eq:4}
  \Pr(default) = \frac{y - r}{1 - R}
\end{equation}
where $y$ is the yield of the risky bond, $r$ is the yield of a riskless bond with comparable cashflows, and $R$ is the the recovery value of the bond in the event of the default as a proportion of the face value of the bond.
For the purposes of this, the ``default'' event is war. 
For the ``riskless'' yield, the yield of the bond during a period in which peace is assumed is used. 
...
For each bond implied probability of war is calculated using equation~\eqref{eq:4}.
The implied probabilities are plotted in Figure~\ref{fig:prwar}.
