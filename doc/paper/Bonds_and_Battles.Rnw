...
\footnote{
  ...
  The Sixes of 1868 include two issues: the Loan of 1847 (Act of January 28, 1847), and the Loan of 1848 (Act of March, 1847) \parencites[72-73,145-148]{united1882national}[72-74]{ustreasury1900}.Bont o
...
The Sixes of 1868 are used for dates less than September 1st, 1861 and the Sixes of 1881 are used thereafter.
...
At that time the Sixes of 1868 and 1881 had similar prices; On August 27, 1861, the Sixes of 1868 were quoted at 87.5, while on September 3, 1861, the Sixes of 1881 were quoted at 89.125.
}
...
local({
    prices0 <- filter(PROJ[["db"]][["prices0"]],
                      src == "Bankers",
                      date >= as.Date("1860-9-1") & date <= as.Date("1861-1-1"),
...
@
\caption{Yields of northern, southern, and government bonds 1855-1865}
\label{fig:yields1855-1865}
\end{figure}

\subsection{Ex Ante Probability of War}

The \textit{ex ante} probability of war can be calculated by treating the war as a default event.
A common approximation of the probability of the default risk of a bond is
\begin{equation}
  \label{eq:4}
  \Pr(\text{default}) = \frac{y - r}{1 - R}
\end{equation}
where $y$ is the yield of the risky bond, $r$ is the yield of a riskless bond with comparable cashflows, and $R$ is the the recovery value of the bond in the event of the default as a proportion of the face value of the bond \parencite{Hull2004}.

Equation~\eqref{eq:4} can be used to estimate the probability of war.
In this case $y$ is the yield of the bond. 
In this case instead $r$ being the yield of a risk free bond, $r$ is the yield of the bond if there was no risk of war.
In other words, $r$ may still have risk due to other factors, but not the risk due to war.
The estimate of this war riskless yield will be to use the average yield for an earlier period in which the risk of war closer to zero.
In this case the 

The recovery rate $R$ is the value or price of the bond if the war occurs.
Since the war did occur, the payment in the event of war is known and can used in the equation.
The recovery in the event of war is assumed to be the present value of the bond assuming the yield of the bond observed at the start of the war.
Let $y_{w}$ be the average yield of the asset in the month after the war started, this is the yield that incorporates the risk associated with expectations of that war at the start of the war.
Then $R$ is the present value of the bond at time $t$ assuming an interest rate of $y_{w}$.

For these calculations, only government and southern state bonds were used; northern state bonds do not react much to the start of the war.

For each bond, the implied probability of war is calculated using equation~\eqref{eq:4} and plotted in Figure~\ref{fig:prwar}.
For the southern state bonds, the implied probability of war is approximately zero, meaning that the yields on the bonds during these periods was the same as, or in many cases, less than the yields during the benchmark period.

The implied probabilities of war do not move much above zero until November 1860, with Lincoln's election.
For the most of the bonds, the implied probabilities of war leading up to Fort Sumter is only between 2.5 and 5 percent.
The U.S. bonds have the highest implied probability of war, with the 5's of 1874 implying almost a 10 percent probability of war and the 6's of 1868 implying a 20-25 percent.

Thus, although the market responded to the increasing risk of war, it did not respond until the election of Lincoln and even after the election generally discounted the risk of war.

\begin{figure}
  \centering
<<>>=
(ggplot(mutate(PROJ[["db"]][["prwar1"]],
               date = as.Date(date)), 
        aes(x = date, y = prwar3))
 + geom_line()
 + facet_wrap(~ series)
...
\section{War Initiation}

The ex ante probability of war could start.

The ideal way is through a prediction market. 
Example from Iraq.

Without a prediction market, a prices of a relevant financial market asset can still provide information about the risk of war, although eliciting this information may be more difficult.
Higher yields (lower prices) indicate increased risk. 
Even after accounting for other factors that may influence the asset risk, increases in yield could indicate either increased risk of war, or increased expectation of the cost of war.

This provides some insight into commitment versus private information theories of war. 
In a commitment theory of war with perfect information most risk of war should have been priced into the asset price.
Under private information theories, the predictability of war is much less.

Let $r_{p}$ be the expected yield under peace, and $r_{w}$ be the expected yield under war.
The price of the bond can be valued as
\begin{align}
  p 
  &= \sum_{i = 1}^{n} c_{i} 
  \left( 
    (1 - \Pr(\mathtt{war} \leq t_{i}) e^{-y_{p} t_{i}} 
    + \Pr(\mathtt{war} \leq t_{i}) e^{-y_{w} t_{i}}
  \right) \\
  &= \sum_{i = 1}^{n} c_{i} 
  \left( 
    (1 - e^{-\lambda t_{i}}) e^{-y_{p} t_{i}} 
    + e^{-\lambda t_{i}} e^{-y_{w} t_{i}}
  \right) \\
\end{align}
Assuming a constant probabilility of war (an exponential distribution with rate $\lambda > 0$).

\begin{align}
  y_{t,j} &= 
  \sum_{i = 1}^{n_{t,j}} c_{t,j,i}
  \left( 
    (1 - e^{-\lambda t_{i}}) e^{-y_{p,j} t_{i}} 
    + e^{-\lambda t_{i}} e^{-y_{w,j} t_{i}}
  \right) + \epsilon_{t,j} \\
  \log \lambda_{t} &\sim N(\log \lambda_{t - 1}, \sigma^{2})
\end{align}
