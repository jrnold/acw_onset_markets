...
While the yields of U.S. government bonds were also stable during this period, their yields wer approximately 0.5 percentage points higher than they had been before the panic.%
%\footnote{The yields of the 6s of 1868 also increased during }
The yields of state bonds were also similar to prior to the panic, and the yields of a few states declined, e.g. Pennsylvania.
...
Lincoln's victory in the presidential election of 1860 (November 6, 1860) precipitated a rise in yields, especially in Southern bonds.
The yields of bonds before and after the election are plotted in Figure~\ref{fig:yields_election}.
For most bonds, there was a slight increase in yields in October before the election.
This rise was likely due to both a monetary contraction as the economy went into recession, but also the anticipation of a Republican victory in the election and the start of a recession.%
...
The start of the recession was marked by a monetary contraction \parencite[413]{BankersMagazine1861}.
There accompanied by monetary contraction and a shortage of specie that likley affected bond prices \parencite[413]{BankersMagazine1861}.
The Bankers' Magazine described the market as,
\begin{quote}
   The market for Government and State loans during the month has been dull. 
   Prices hare In most instances tended downward and a material decline is perceptible in some of the most prominent securities.
...
\begin{quote}
  ...
  \parencites[476-77]{BankersMagazine}
\end{quote}
The market seemed to ant

After Lincoln's election, yields increased greatly, especially for southern states.
This increase did not rise immediately after Licoln's election victory (as would occur with Fort Sumter), but rose over the course of the following month, with some variation between bonds in the timing of the rise.
...
The decline increase was due to a small financial crisis that started when Southern banks started denying credit to Northern clients and withdrew specie from Northern banks \parencite[76]{HomansDana1861a}.
However, the decline was not merely financial, as political risk was the primary concern of the market.
\begin{quote}
  By the 18th a very unfavorable condition of the market was developed.
  The political complications in South Carolina had engendered a feeling of distrust in the stability of the present union of the States and public and private credit received a severe shock.  \parencite[501]{BankersMagazine1861}
\end{quote}
...
The spread (difference) between the yields of southern and northern state bonds is a measure of the market's expectation of war.%
\footnote{
...
Although war could increase the riskiness of all state bonds, there are at least two ways in which war would affect the increase the risk of southern bonds relative to northern state bonds.
...
Second, it is likely that a war would be fought primarily in southern states, and thus southern states would incur more destruction that could inhibit their ability to repay debt in the future.
The differential effect the war to the risk of southern and northern bonds was observed in the changes in the yields of these bonds after Fort Sumter, shown in Table~\ref{tab:sumter}.
In the week after Fort Sumter, the yields of southern states increased 3--8 percentage points, while the yields of northern states increased only 0.01 to 0.35 percentage points.
Using the spread rather than simply using yields controls for shocks common to all state bonds.
This does not eliminate common factors to southern bonds, 
...
During the period close to the start of the American Civil War, it is likely that war risk would be the most influential factor influencing the spread, and certainly the most plausible factor for sudden changes in the spread.
...
Since the number of bonds in each period is relatively small, the parameters $\theta$ and $\psi$ are given random walk priors.
However, the estimation should not overly smooth these parameters since one of the purposes of this work is to identify key events.
Thus, for these parameters, a horseshoe prior distribution is used for the innovations \parencites{CarvalhoPolsonScott2009}{CarvalhoPolsonScott2009}.
This is a shrinkage prior, similar to the LASSO \parencites{}, and produce results similar to a change-point model in which the number of changes is estimated from the data.
\begin{align}
...
Figure \ref{fig:north_south_spreads} plots estimates from the posterior distribution of the south-north spread ($\psi_{t}$).
Over the entire period considered, the spread was positive, meaning that southern state bonds were riskier on average than northern state bonds, but this could be due to a number of factors other than war risk.
...
\begin{align}
  r_{j} & \sim \dlnorm{\bar{r}_{j},s_{j}}
\end{align}
where $\dlnorm{m, s}$ is the log-normal distribution with location $m$ and scale $s$, $\bar{r}_{j}$, $s_{j}$ are the mean and standard deviation of the logarithm of yields for bond $j$ for the period, \Sexpr{DATEFMT_PEACE_START}  to \Sexpr{DATEFMT_PEACE_END}.
...
(ggplot(filter(PROJ$db[["prwar3_summary"]][["phi"]],
               date >= as.Date("1858-7-1")
               ),
        aes(x = date))
 + geom_vline(xintercept = as.numeric(DATE_ELECTION), colour = "gray")
