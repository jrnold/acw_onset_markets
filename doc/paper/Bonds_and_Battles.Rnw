...
local({
    (ggplot(filter(PROJ$db[["spreads1_summary"]][["theta"]],
                   dimension == 2),
            aes(x = date))
     + geom_ribbon(mapping = aes(ymin = p025, ymax = p975), alpha = 0.1)
...
Using this spread rather than the raw yields, has the advantage of controlling for factors which have the same influence on all state bonds.
Although the war could affect the risk of all bonds, it would have differential effects on the risks of southern and northern state bonds.
This was observed after Fort Sumter, in which the yields of southern states increased 3-8 percentage points, while the yields of northern states increased only 0.01 to 0.35 percentage points (Table \ref{tab:sumter}).%
...
This does not isolate war risk, as there are potentially other factors which could have differential effects, but in this period they are less likely and less prominent than war risk.
As will be noted later, while the Panic of 1857 had a large  had the largest effects in the midwest, yet the yield between northern and southern bonds did not change that much.
...
Figure \ref{fig:yields_regions} plots the yields of northern and southern bonds and Figure \ref{fig:north_south_spreads} plots the difference between the mean yields of southern and northern state bonds as estimated with the model described in section \ref{sec:model-south-north-yield-spreads}.

Over the entire period considered here, the 

From 1855 to the Panic of 1857, the spread was small.
...
From the Panic of 1857 until mid 1860, the average spread was stable but slightly higher than period before the Panic (range.
Although the uncertainty over the spread is less than before the Panic of 1857, the interval still includes zero.
The spread starts to increase in the second half of 1860, increasing sharply after the election of Lincoln, and reaching ... during January to April 1861.
...
local({
    (ggplot(PROJ$db[["prwar2_summary"]][["phi"]],
            aes(x = date))
     + geom_ribbon(mapping = aes(ymin = p025, ymax = p975), alpha = 0.1)
     + geom_ribbon(mapping = aes(ymin = p25, ymax = p75), alpha = 0.5)
     + geom_line(mapping = aes(y = mean))
...
@ 
  \caption{
    Implied probabilities of the initiation of war calculated from U.S. government and southern and border state bonds, \Sexpr{DATEFMT_RANGE_START} to \Sexpr{DATEFMT_RANGE_END}, using the model described in section \ref{sec:model-war-probability}
    The line is the posterior mean, the dark ribbon is the 50\% credible interval, the light ribbon is the 95\% credible interval.
  }
  \label{fig:prwar}
\end{figure}
...
This is a statistical model of bond yields of state bonds.
...
The parameters $\theta$ and $\psi$ evolve as random walks, but with horseshoe prior \parencite{CarvalhoPolsonScott2010} innovations to allow for large or sparse changes in the level of the factors.
\begin{align*}
