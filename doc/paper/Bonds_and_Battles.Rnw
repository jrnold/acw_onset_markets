...
<<knit_opts, results = 'hide', echo = FALSE>>=
...
<<header,results = 'hide',echo = FALSE, error = FALSE, cache.extra = tools::md5sum("../../.Rprofile")>>=
...
<<war_initiation, results='hide', cache.extra = tools::md5sum("filehashdb/prices0")>>=
...
<<south_north_spread_data, results = 'hide', cache.extra = tools::md5sum(file.path("filehashdb", "spreads2_summary"))>>=
...
<<prob_war_data, cache.extra = tools::md5sum(file.path("filehashdb", c("prwar1", "prwar4_summary")))>>=
...
The estimated implied probability of war immediately before the Battle of Fort Sumter to be between \Sexpr{prwar_18610413_low} and \Sexpr{prwar_18610413_high} percent.
\end{abstract}
...
\footnote{Interpret strings of the form ``\Sexpr{prwar_18610413}'' as ``mean (2.5\% quantile, 97.5\% quantile)''}
...
\begin{figure}
  \centerfloat
<<fig_yields_regions, fig.height = 8, fig.width = 6>>=  
local({
