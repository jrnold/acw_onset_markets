...
pre_panic_means_min <- pre_panic_means[1, "ytm_mean"]
pre_panic_means_min_name <- pre_panic_means[1, "series"]
pre_panic_means_max <- pre_panic_means[nrow(pre_panic_means), "ytm_mean"]
...
State bond yields were also relatively stable, although having higher yields than government bonds; with average yields between \Sexpr{pre_panic_mean_min} (\Sexpr{pre_panic_mean_min_name}) and \Sexpr{pre_panic_mean_max} (\Sexpr{pre_panic_mean_max_name}).
...
\subsubsection{Probability of War}
