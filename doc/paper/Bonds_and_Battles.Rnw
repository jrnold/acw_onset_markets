...
I find that, on average, Confederate victories were associated with decreases in the bond price and Union victories associated with  incnreases.
...
@
\caption{Yields of Southern State and U.S. Government bonds before and after the Election of 1860.}
...
local({
    ...
    x <- mutate(merge(x, prwar1[["yields"]][ , c("series", "yield_peace", "price_war")]),
                yield_peace = yield_peace * 100)
    rownames(x) <- x[["series"]]
    x[["series"]] <- NULL
    colnames(x) <- c("Yield Pre-war", "Yield Post-war",
                     "$\\Delta$ yield",
                     "\\parbox{1.5cm}{Avg. yield \\\\  (1855-1857)}", "Price war")
...
\caption{Yields of Southern and U.S. Government bonds before and after the Battle of Fort Sumter.
  ...
  Peace yields are the average yield from \Sexpr{PEACE_START} to \Sexpr{PEACE_END}.
  The war price (average price from \Sexpr{WAR_START} to \Sexpr{WAR_END}).}
\label{tab:yields_pre_post_war}
\end{table}

To what extent did financial markets anticipate the start of the American civil war? 
How did the market's assesment of war risk change leading up to the war? 
Which events were associated with these changes?
This section answers these questions using data on the prices and yields of U.S. government and state bonds.
Three measures are used: the bond yields, the spread between northern and southern bonds, and the implied probability of war calculated by treating war as a default event.
From these data, it appears that market believed there was only a negligible risk of war prior to the election of Lincoln in November 1860.
After the election and before Fort Sumter, the market assigned a higher risk of war, but it risk of war was still believed to be low right up to the start of the war.

To assess financial market beliefs about the \textit{ex ante} risk of the  American Civil War, this paper uses the prices and yields of U.S. government and state bonds, both northern and southern, traded in New York City from 1855-1861.
The data consist of bonds issued by nine states (Georgia, Indiana, Kentucky, Louisiana, Missouri, North Carolina, Ohio, Pennsylvania and Virginia) and two bonds issued by the US Government (6s of 1868, 5s of 1874).
\footnote{In the jargon of that era, 5s and 6s refer to bonds paying 5 percent and 6 percent interest, respectively.}
The set of bonds included here are those with regular quotations in the \textit{Bankers' Magazine}, one of the leading trade magazines of that era.
Prices of these bonds are available at approximately a weekly frequency from July 1855 through the start of the war in April 1861.
...
\footnote{
  In the quotations given in the \textit{Bankers' Magazine}, the maturity of most state bond issues was not given. 
  Other sources were used to find the likely bond issues to which the quotations referred to. 
  When there was uncertainty or multiple maturities, the yield to maturity was calculated for each maturity, and the average yield over all maturities is used.
  See the data documentation for details.
}
The data consist of only two U.S. government bonds, the 6s of 1868 and the 5s of 1874, because these were the only two large debt issues of the U.S. outstanding at the start of the the civil war.%
Between the War of 1812 and the Civil War, the U.S. government only infrequently issued debt; the 6s of 1868 were issued in 1848 to pay for the Mexican-American War, while 5s of 1874 were issued in 1858 due to a funding shortfall as a result of a financial crisis, the Panic of 1857  \parencite{HomerSylla2005}.
The state bonds included here were mostly issued by southern and midwestern states in order fund infrastructure projects, such as railroads or canals (NEED CITATION).
During this period, most New England states did not issue debt, with the notable exception of Massachusetts, whose bonds were the essentially the risk-free asset of the period, but were not regularly quoted in the \textit{Bankers' Magazine}.
\footnote{
  Prior to 1860, only Massachusetts, Maine and Vermont had public debt, and the amount of debt of the later two was quite small. \parencite[86]{Martin1871}.
...
<<fig.height = 8, fig.width = 6>>=  
local({
    .data <- filter(PROJ[["db"]][["prices0"]],
                    src == "Bankers",
                    date >= as.Date("1855-7-1"),
                    date < prwar1[["war_dates"]][1],
                    ! grepl("California", series))
    .data2 <- plyr::ldply(sort(as.character(unique(.data[["series"]]))), function(i) {
        .data[["i"]] <- i
    ...
    (ggplot(.data2,
            aes(x = date, y = ytm * 100, group = series, color = series_is_i))
     + geom_line()
     + facet_wrap(~ i, ncol = 2)
     + scale_color_manual(values = alpha("black", c(0.2, 1)))
...
@
\caption{Yields of U.S. government and state bonds, by bond, July 1855 to May 1861.}
\label{fig:yields_issuer}
\end{figure}
...
local({
    ...
    #.data[.data[["series"]] == "Kentucky 6s", "group"] <- "North"
    .data2 <- plyr::ldply(c("North", "South", "Union"), function(i) {
        .data[["i"]] <- i
        .data[["group_is_i"]] <- .data[["group"]] == i
        .data
    })
    (ggplot(.data2,
...
@
\caption{Yields of U.S. government and state bonds, by region, July 1855 to May 1861.}
\label{fig:yields_regions}
\end{figure}

Figure~\ref{fig:yields1855-1865} plots the yields of all the bonds in the data.
Informally, the yields during 1855-1861 can be divided into four periods: July 1855 to October 1857,
November 1857 to January 1858 (the Panic of 1857), February 1858-October 1860, November 1860-April 1861.

From July 1855 through October 1857, yields were low and stable.
The yields of the U.S. 6s of 1868 were between ... and ....
These were the lowest yields on U.S. government bonds since the 1789 \parencite[282-283]{HomerSylla2005}.
These yields were even lower than the safe Massachusetts and Boston city, and only one percentage point higher than the yield on 3\% British consols, which was considered the safest global asset during that period.%
State bond yields were also relatively stable, although higher than government bonds.
State bonds were pereceived to be riskier assets, since several states had defaulted in the 1840s \parencite{English1996}.
Yet the yields of the state bonds during this period were much lower than in the 1840s, when some states had yields of 

From October 1857 to January 1858, yields spiked during the Panic of 1857.
The Panic of 1857 was a recession and financial crisis that included the failure of a major insurance company, the bankruptcy of several railroads, and the suspension of specie payments by banks in New York City. 
The causes of the panic were not directly related to political risks of secession or war, but were likely a decline in European demand for U.S. trade or insufficient specie supply to meet demand due to a decline in gold production \parencites[263-265]{Dewey1918}[277,299]{HomerSylla2005}.
\footnote{The \textit{Bankers' Magazine} money sections do not mention domestic political issues as a cause of the crisis, attributing it to a decline in foreign trade and bad banking}
So although yields reached high levels the panic, this should not be interpreted as representing an increase in war risk.
    
After the Panic of 1857, from February 1858 through mid 1860, yields of bonds returned to low and stable levels similar to those prior to the panic.
The yields of U.S. government bonds were also largely stable, although approximately 0.5 percentage points higher than they had been before the panic.
The yields of southern state bonds were approximately the same as prior to the panic, while the yields of a few northern states, e.g. Pennsylvania, declined.
Towards the end of this period, July-October 1860, there appears to be a slight and steady increase in yields. ``The continued contraction of the banks, and the decrease in the specie reserve, caused some activity in the money market in the beginning of the month...'' \parencite{BankersMagazine1861}
The relatively low and stable yields during this period suggest that there was perceived to be little risk for these bonds, or at least risk not much different than the period 1855-1857, let alone war risk.

The Lincoln's victory in the election of 1860,
\footnote{
  The election was held on November 6, 1860. 
  Although the election was close, Lincoln's victory was believed to be extremely likely the next day, and certain by the 8th. CITE NYT ARTICLES.
}
The election of 1860 precipitated a financial crisis, as soon after the election Southern banks began denying credit to Northern merchants and investors and withrew specie from Northern banks.
\begin{quote}
  In this state of affairs the results of the Presidential election came with a blight upon the market. 
  Threats of disunion caused an alarm to which the banks were the first to give practical effect by curtailing their discounts at the time when the community required expansion. 
  The banks at the South first refused to discount the usual cotton drafts payable after the 1st of November.
  The New York banks at once held up their lines of discounts, the money pressure became intense, sterling bills could not be sold, houses under Western drafts were thus heavily embarrassed, and exchange all over the country rose rapidly on New York. 
  All the wheels of commerce were becoming clogged, and the danger of a general suspension of the merchants was imminent. 
  \parencite[76]{HomansDana1861a}
\end{quote}
Over the following months, yields stayed high and were variable as news  \parencites[718]{HuntKettellHomansEtAl1860}[78,198,416,677]{HomansDana1861a}[413,481,669,756,838]{BankersMagazine1861}.
%% Although North Carolina, Tennessee, and Virigina did not formally join the Confederacy until after Fort Sumter (May 20, May 7 and April 17, respectively), their bonds increased in yield after the election of 1860.
%% However, the yield of the Georgia bonds did not increase until the state seceeded in January.
The increase in yields after the election suggests that either the likelihood of Lincoln's election or the response of the Southern states to the election were uncertain before the election. 
Prior to the election there was some slight increase in yields, but the month before the election the \textit{Bankers' Magazine} wrote,
\begin{quote}
  The money market has been active and comparatively easy during the past month.
  There have been no serious causes of disturbance in and around Wall street.
  \parencites{BankersMagazine1861}
\end{quote}

\begin{quote}
   The news was also of a character to affect disastrously American
   credit, since it showed a decline of 13 per cent iu United States
   government stocks, solely from fear of disunion, which event would
   reduce the States to the condition of the bankrupt, "disorderly
   houses" of South America, and leave the separate States a prey to
   every spoiler.
   \textit{Merchants' Magazine, January 1861, p. 78}
\end{quote}

\begin{quote}
  The only speck in the horizon is the threat of secession in the
  South. We would be the last to give countenance to the libel upon the
  South, that the majority of any one State assented to the proposition
  of secession. The permanent social, commercial and financial interests
  of the various sections of the whole country are too strongly
  identified—too strongly bound together—to induce the loss of any one
  State; \parencites[417]{BankersMagazine1861}
\end{quote}
...
Even on the eve of the war, the market appears to have largely expected some sort of peaceful settlement to the crisis.
...
The start of the war resulted in a `` depreciation in Southern State stocks exceeding any decline ever witnessed at the stock board of this city.'' \parencite[919]{BankersMagazine1861}.
...
<<spreads_data, results = 'hide'>>=
prices <-
    (mutate(filter(PROJ[["db"]][["prices0"]],
                   src == "Bankers",
                   date >= as.Date("1855-7-1"),
                   date <= as.Date("1861-5-18"),                       
                   ! grepl("California", series)),
            ytm = ytm * 100)
     )
#.prices[.prices[["series"]] == "Kentucky 6s", "group"] <- "North"
prices_north <- 
    (filter(prices, group == "North")
     %>% group_by(date)
     %>% dplyr::summarise(north_mean = mean(ytm),
                          north_var = var(ytm)))
prices_south <- 
    (filter(prices, group == "South")
     %>% group_by(date)
     %>% dplyr::summarise(south_mean = mean(ytm),
                          south_var = var(ytm)))
prices_by_group <-
    mutate(merge(prices_north, prices_south),
           diff_mean = south_mean - north_mean,
           diff_var = south_var + north_var,
           diff_lb = diff_mean - 2 * sqrt(diff_var),
           diff_ub = diff_mean + 2 * sqrt(diff_var))

spread_1855_1857 <-
    (filter(prices_by_group, date <= as.Date("1857-10-1"))
     %>% summarise(lb = min(diff_mean),
                   ub = max(diff_mean)))

spread_1858_1860 <-
    (filter(prices_by_group, 
            date >= as.Date("1858-3-1"),
            date <= as.Date("1860-7-1"))
     %>% summarise(lb = min(diff_mean),
                   ub = max(diff_mean)))
...
\begin{figure}
  \centerfloat
<<north_south_spreads, fig.height = 3, fig.width = 6>>=
local({
    ## .all_diff <-
    ##     plyr::ddply(filter(.prices, group %in% c("North", "South")), "date",
    ##                 function(x) {
    ##                     north <- filter(x, group == "North")[["ytm"]]
    ##                     south <- filter(x, group == "South")[["ytm"]]
    ##                     ns <- 
    ##                         mutate(expand.grid(north = north, south = south),
    ##                                diff = south - north)
    ##                 })
    ## .mean_diff <- 
    ##     (group_by(.all_diff, date)
    ##      %>% dplyr::summarise(spread = south_north_spread))
    ## (ggplot()
    ##  + geom_point(data = .all_diff,
    ##               mapping = aes(x = date, y = diff), 
    ##               alpha = 0.15)
    ##  + geom_line(data = .mean_spread,
    ##              mapping = aes(x = date, y = south_north_spread),
    ##              colour = "red")
    ##  + geom_hline(yintercept = 0)
    ##  + scale_x_date("", breaks = "years", labels = date_format("%Y"))
    ##  + scale_y_continuous("spread (%)", breaks = seq(-2, 12, by = 2))
    ##  + theme_local()
    ##  )
    (ggplot()
     + geom_ribbon(data = prices_by_group,
                   mapping = aes(x = date, ymin = diff_lb, ymax = diff_ub),
                   fill = "gray")
     + geom_line(data = prices_by_group,
                 mapping = aes(x = date, y = diff_mean),
                 colour = "black")
     + geom_hline(yintercept = 0)
     + scale_x_date("", breaks = "years", labels = date_format("%Y"))
     + scale_y_continuous("spread (%)", breaks = seq(-2, 12, by = 2))
...
@ 
\caption{Mean spread between the yields of southern and northern state bonds.
  The line is $\bar y_{S-N} = \bar y_{S} - \bar y_{N}$ and $\bar y_{i}$ is the sample mean of region $i$.
  The shaded area is $\bar y_{N} \pm 2 \cdot s_{S - N}$ where $s_{S - N} = \sqrt{s_{S}^{2} + s_{N}^{2}}$ and $s_{i}$ is the sample standard deviation of region $i$. 
  The northern states are Indiana, Ohio and Pennsylvania; the southern states are Georgia, Louisiana, North Carolina, Tennessee and Virginia. 
  The border states (Kentucky, Missouri) were excluded.
  }
\label{fig:north_south_spreads}
\end{figure}

An issue with looking at the yields of state and government bonds makes is that is confounds non-war factors influening the yields of the bonds with war risk factors.
One way to control for some, but not all, of the non-war risk factors is to look at the the spread between northern and and southern state bond yields.
Using only state bonds controls for shocks to the risk of all state bonds.
However, the secession of southern states and war would increase the risk of southern bonds relative to the northern bonds.
Figure \ref{fig:north_south_spread} plots the difference between the mean yields of southern and northern state bonds.

From 1855 to the Panic of 1857, the spread was \Sexpr{format(round(spread_1855_1857[["lb"]],2))} to \Sexpr{format(round(spread_1855_1857[["ub"]], 2))}.
In the Panic of 1857, the spreads between southern and northern bonds increased only slightly; although the financial crisis increased the yields of all state bonds, see Figure \ref{fig:yields_issuer}, it increased all the yields similarly.
From the Panic of 1857 until mid 1860, the average spread was stable but slightly higher than period before the Panic (\Sexpr{format(round(spread_1858_1860[["lb"]], 2))} to \Sexpr{format(round(spread_1858_1860[["ub"]], 2))}.
Although the uncertainty over the spread is less than before the Panic of 1857, the interval sill includes zero.
The spread starts to increase in the second half of 1860, increasing sharply after the election of Lincoln, and reaching \Sexpr{format(round(spread_1861[["lb"]], 2))} to \Sexpr{format(round(spread_1861[["ub"]], 2))} during January to April 1861.
Note that this increase in spread is different than the Panic of 1857, in which yields increased but the South-North yield spread did not.
After Fort Sumter, the spread increased 4 to 6 percentage points.
Thus, not only did yields sharply increase after Fort Sumter, the spread between southern and northern state bonds increased sharply, as noted by the contemporary financial press, ``The stock market has been variable during the month. The prominent features being an unexpected firmness (events considered) in \dots{} Northern State stocks \dots{}, and a depreciation in Southern State stocks exceeding any decline ever witnessed at the stock board of this city \parencite[919]{BankersMagazine1861}''


Although the yields and spread indicate changes in the risk leading up to the war, they are not directly interpretable as the probability of war.%
However, the market's belief of the probability of war can be inferred from bond prices or yields by treating war as a default event and using methods for extracting the probability of default from bond prices or yields.
A simple and common approximation for the probability of the default of a risky bond using its yield is,
\begin{equation}
  \label{eq:4}
...
Equation~\eqref{eq:4} can be to estimate the probability of war by properly defining $r$ and $R$.
For the case of estimating the probability of war, the risk-free interest rate $r$ is the yield of a comparable bond with no-risk of war.
This means that the yield $r$ will not necessarily be risk-free, and should still include any risks not due to war while excluding risks due to war.
One estimate of this war risk free yield is the the average yield for the same bond for an earlier period in which the risk of war is approximately zero.%
\footnote{
  ...
  Additionally, all of the bonds considered here have relatively long maturities during the period considered, and in light of no other information about the yield curve, a flat yield curve is reasonable.
}
For each bond considered here, $r$ is the mean yield from \Sexpr{PEACE_START} to \Sexpr{PEACE_END}.%
\footnote{The 5s of 1874 were not issued until 1858, but given the similarity of the yield to the 6s of 1868 the $r$ of the 6s of 1868 is used for the 5s of 1874}
...
\footnote{
  During this era, 3\% British consols yielded 3.22-3.31 percent \parencite[193]{HomerSylla2005}.
  During this period, state bonds  had higher interest rates due to a series of defaults in the 1840s \parencite[320]{HomerSylla2005}.
}

The recovery rate $R$ is the amount, as a proportion of face value (or future payments), should be the amount investor receives in the event of war.
Since the American Civil War did occur, $R$ is known and the price of the bonds observated at the start of the war can be used.
...
Figure~\ref{fig:prwar} displays the implied probabilities of war for southern and border state and U.S. government bonds for the period after the Panic of 1857 to the Battle of Fort Sumter, \Sexpr{RANGE_START} to \Sexpr{RANGE_END}.
...
\begin{figure}
  \centerfloat
<<prwar, fig.height = 8, fig.width = 6>>=
local({
