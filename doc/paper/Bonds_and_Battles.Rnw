...
Using the prices and yields of ante-bellum U.S. government and state bonds, this section addresses these issues with three methods: the overall yields of these bonds, the spread between northern and southern state bond yields, and by calculated the implied probability of war using methods for calculating the probability of default.
The overall story from these analyses is that the market prices imply a negligable risk of war prior to Lincoln's election in November 1860, and although they responded to the increased risk of war between Lincoln's election and Fort Sumter, the implied risk of war was still quite low.
...
The data consist of bonds from ten states (Georgia, Indiana, Kentucky, Louisiana, Missouri, North Carolina, Ohio, Pennsylvania and Virginia) and 2 bonds issued by the US Government (6s of 1868, 5s of 1874).
All of these bonds are coupon bonds (5-6 percent coupons paid semi-annually) with longer maturities; the U.S. bonds matured in 1868 and 1874 respectively, while the state bonds variously matured between 1869 and 1892.
...
@
\caption{Yields of U.S. government and state bonds, 1855-1865.}
...
\begin{itemize}
\item From 1855 to October 1857, yields were largely stable.
  The yields of state bonds were between 5--8 percent.
  The yields of the U.S. 6s of 1868 were stable at ... to ... .
  This was the lowest yield of U.S. bonds up to that point.
\item From October 1857 to January 1858, yields of state and government bond spiked.
  This was due to the Panic of 1857. 
  DESCRIPTION.
\item From February 1858 to November 1860, the prices of state bonds recovered to where they were before the Panic of 1857, and in several cases were less than yields during that period.
\item After Lincoln won the election in November 1860, yields started to increase, although with varying speeds. 
  ...
  INCLUDE QUOTES.
\item The start of the war resulted in a `` depreciation in Southern State stocks exceeding any decline ever witnessed at the stock board of this city.'' \parencite[919]{BankersMagazine1861}.
\end{itemize}
...
local({
    ## (ggplot(filter(PROJ[["db"]][["prices0"]],
    ##               src == "Bankers",
    ##               date >= as.Date("1855-7-1"),
    ##                date <= as.Date("1861-7-1"),
    ##               ! grepl("California", series)),
    ##        aes(x = date, y = ytm, colour = group, group = series))
    ##  + geom_line())
    .data <-
        (filter(PROJ[["db"]][["prices0"]],
                src == "Bankers",
                ...
                ! grepl("California", series)))
    .data[.data[["series"]] == "Kentucky 6s", "group"] <- "North"
    .data <- (group_by(.data, group, date)
              %>% dplyr::summarise(yield_arith = mean(ytm * 100), 
                                   yield_geom = gmean(ytm * 100))
              )
    .data2 <- 
        (dcast(.data, date ~ group, value.var = "yield_geom")
...
Figure \ref{fig:prewarspreads} plots the difference between the average yield of southern state bonds and nothern state bonds.
