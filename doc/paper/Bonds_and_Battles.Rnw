...
The market's assessment of the risk of war can also be seen in the spread (difference) between the average yields of southern and northern state bonds.
Using this spread rather than the yields themselves, has the advantage of controlling for factors which have the same influence on all state bonds.
War would have differential effects on northern and southern state bonds, as observed after Fort Sumter in which the yields of southern states increased 3-8 percentage points, while the yields of northern states increased only 0.01 to 0.35 percentage points.
Thus the expectations of the probability of war will have a larger (positive) effect on the yields of southern bonds than on the yields of northern bonds, and thus as the perceived risk of war increased the spread between southern and northern bonds should also increase.
There are potentially other factors which would have these effects, but they are less likely and less prominent than war risk.
For example, the Panic of 1857 had the largest effects in the midwest, yet the yield between northern and southern bonds did not change that much.
The most immediate differential effect of the war on southern state bond holders in the north was that the states stopped paying interest payments.%
\footnote{It seems that after the war, southern states repaid debt issued \textit{before} the war in some manner; though all defaulted on any debts issued while part of the Confederacy, and some defaulted on debt issued by reconstruction governments after the war. CITE}

Figure \ref{fig:yields_region} plots the yields of northern and southern bonds.
Figure \ref{fig:north_south_spreads} plots the difference between the mean yields of southern and northern state bonds.
The statistical model used to estimate the spread is described in ...
