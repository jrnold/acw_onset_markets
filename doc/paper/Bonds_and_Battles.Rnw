...
The \textit{Annual Report} estimated that revenue to pay the expecpted expenditures should be: 80 million in taxes, and 240 million by the issue of new loans  \parencite[6]{Treasury1861a}.
The Act of July 17, 1861 was the first act passed by Congress to fund the war.
It authorized the issue of \$250 million in debt \parencite[44]{Treasury1863}.
Since  the debt on July 1, 1861 was 90 million \parencite[23]{Treasury1861}, this represented an increase of 178\%.
By comparison, the debt at the end of the War of 1816 was 127 million \parencite[29]{Elder1863} and the 49 million was issued to fund the Mexican American War.%
...
The spread (difference) between the yields of southern and northern state bonds is another measure of market beliefs of war risk.
...
The realization of the start of the war showed that this was the case; in which the yields of southern states increased 3--8 percentage points, while the yields of northern states increased only 0.01 to 0.35 percentage points, see Table \ref{tab:sumter}.
The most immediate difference between northern and southern state bonds for northern investors, is that at the start of the war southern state bonds immediately stopped paying interest 
Also, the Civil War was largely fought in the southern states, so they suffered much more damage than Nothern states; if the market had any expecation of this, it would also contribute to the southern state bonds being more riksy.
Since war would have a differential effect on the risk of northern and southern state bonds, as the risk of war increases, the spread between southern and northern state bonds should increase.

Using the spread rather than directly using the yields of states has the advantage of controlling for shocks which influenced both northern and southern state bonds. 
A reason for analyzing the spread between southern and northern state bonds instead of the spread against U.S. government bonds is that the the state bonds likely have more similar common shocks.
There may other state-level factors which result in risk differences between northern and southern states.
Hoever, focusing on changes in the spread rather than the level of the spread controls for any of these regional factors which are constant over the time period considered.
During the period considered, it is likely that war risk would be the most influential factor influencing the spread, and certainly the most plausible factor for sudden changes in the spread.

Figure \ref{fig:yields_regions} plots the yields of bonds with the yields in each group highlighted.
The objective is to estimate the 
The south-north spread is estimated using a statistical model of the yields of the northern andd southern state bonds.
Let $y_{j,t}$ be the yield for bond $j$ at time $t$.
The yield is modeled as the sum of the market yield for all state bonds, $\theta$, and the spread between southern and northern state bonds,
\begin{equation}
  y_{j,t} \sim \dnorm{\theta_{t} + \psi_{t} \mathtt{south}_{j}, \sigma^{2}}
\end{equation}
where $\mathtt{south_{j}}$ is a binary variable indicating whether the bond was issued by a southern state.
The values of the south-north spread $\psi_{t}$ are of primary interest.

Since the number of bonds in each period is relatively small, in order to dampen fluctiuations that may occur by randomness, the parameters $\theta$ and $\psi$ are given random walk priors to pool data over time.
However, estimation should not over-smooth these parameter, especially since they will be large fluctations in these parameters around events like the election of 1860.
Thus, for these parameters, a horseshoe prior distribution \parencite{CarvalhoPolsonScott2010} is used for the innovations.
This is a shrinkage prior, similar to the LASSO, and produce results similar to a change-point model in which the number of changes is estimated from the data,
\begin{align}
...
The data used to calculate the south-north spread consists of the yields of northern (Indiana, Ohio and Pennsylvania) and southern (Georgia, Louisiana, North Carolina, Tennessee and Virginia) state bonds from July 1st, 1855 to June 1st, 1861.
\footnote{The border states of Kentucky and Missouri are not included in these calculations.}
Figure \ref{fig:north_south_spreads} plots the estimated south-north spread ($\psi_{t}$).
...
\subsubsection{South-North Yield Spreads}
