...
prices_prewar_period_bond <-
    (group_by(prices_prewar, period, series)
     %>% dplyr::summarise(ytm_mean = mean(ytm),
                          ...
                          price_min = min(price),
                          price_max = max(price)))
rownames(prices_prewar_period_bond) <- paste(prices_prewar_period_bond$series, 
                                             prices_prewar_period_bond$period, sep = ":")

prices_sumter <- 
    local({
        .data <- mutate(filter(prices_prewar,
                               date %in% c(DATE_WEEK_PRE_SUMTER, DATE_WEEK_POST_SUMTER)),
                        date = format(date, "%b %e, %Y"))
        .data <- mutate(dcast(.data, series ~ date, value.var = "ytm"),
                        `change` = `Apr 20, 1861` - `Apr 13, 1861`,
                        `pct. change` = (`Apr 20, 1861` - `Apr 13, 1861`) / `Apr 20, 1861`)
              ## mutate(dcast(.data, series ~ date, value.var = "price"),
              ##        `change` = `1861-04-20` - `1861-04-13`,
              ##        `pct. change` = (`1861-04-20` - `1861-04-13`) / `1861-04-13`))
...
local({
    ...
    prewar <- 
        (group_by(filter(prices, 
                         date >= DATE_PREWAR,
                         date < DATE_FORT_SUMTER),
                  series)
         %>% dplyr::summarise(yield_pre_war = mean(ytm)))
    postwar <- 
        (group_by(filter(prices, 
                         date >= prwar1[["war_dates"]][1],
                         date <= prwar1[["war_dates"]][2]),
                  series)
         %>% dplyr::summarise(yield_post_war = mean(ytm)))
...
(ggplot(prices_prewar, aes(x = period, y = ytm))
 + geom_point()
 + facet_wrap(~ series, ncol = 2))

@ 

\caption{Summary of yields of U.S. government and state bonds prior to and at the start of the American Civil War}
...
@
\caption{Yields to maturity of U.S. government and southern and border state bonds before and after the election of 1860.
  The vertical line indicates the date of the election, November 6, 1860.}
...
@
\caption{Yields to maturity of U.S. government and southern and border state bonds before and after the Battle of Fort Sumter. The vertical line indicates the date when news of the surrender of Fort Sumter reaches New York, April 15, 1861.}  
\label{fig:yields_sumter}
\end{figure}

Figure~\ref{fig:yields_all} plots the yields of all the bonds in the data.
Informally, the yields during 1855-1861 can be divided into five periods: July 1855 to October 1857,
November 1857 to January 1858 (the Panic of 1857), February 1858-October 1860, November 1860-April 14, 1861, and after the start of the war with Fort Sumter (April 14, 1861).

From July 1855 through October 1857, yields were low and stable.
The yields of the U.S. 6s of 1868 ranged between \Sexpr{round(prices_prewar_period_bond["U.S. 6s, 1868:Pre Panic of 1857", "ytm_min"], 2)} and \Sexpr{round(prices_prewar_period_bond["U.S. 6s, 1868:Pre Panic of 1857", "ytm_max"], 2)}.
...
State bond yields were also relatively stable, although higher than government bonds, with an average yield of \Sexpr{round(mean(filter(prices_prewar_period_bond, ! grepl("^U\\.S", series))[["ytm_mean"]]), 2)}.
...
Yet, the yields of the state bonds during this period were much lower than in the 1840s, when some states had yields of above 10 percent and as high as 41 percent \parencite[320]{HomerSylla2005}.
...
\footnote{The \textit{Bankers' Magazine} money sections do not mention domestic political issues during the Panic of 1857, attributing it to a decline in foreign trade and bad banking}
...
The yields of southern state bonds were approximately the same as prior to the panic, while the yields of a few states, e.g. Pennsylvania, declined.
...
The Lincoln's victory in the election of 1860 precipitated a rise in yields, especially in Southern bonds; see Figure~\ref{fig:yields_election}.
...
Some of this was due to a small financial crisis when Southern banks began denying credit to Northern merchants and investors and withrew specie from Northern banks, leading to a minor financial crisis \parencite[76]{HomansDana1861a}.
However, political risk was the primary concern of the market. 
The \textit{Merchants' Magazine} wrote in January 1861,
\begin{quote}
   The news was also of a character to affect disastrously American credit, since it showed a decline of 13 per cent in United States government stocks, solely from fear of disunion, which event would reduce the States to the condition of the bankrupt, ``disorderly houses'' of South America, and leave the separate States a prey to every spoiler.
   \textit{Merchants' Magazine, January 1861, p. 78}
\end{quote}
and the \textit{Bankers' Magazine} wrote 
\begin{quote}
...
\end{quote}
Over the following months, yields were remained high, but also variable as events influenced the   \parencites[718]{HuntKettellHomansEtAl1860}[78,198,416,677]{HomansDana1861a}[413,481,669,756,838]{BankersMagazine1861}.%
\footnote{
...
Although Louisiana, North Carolina and Tennessee did not seceed and join the Confederacy until after Fort Sumter, the yields on thos bonds increased during this period.

Although bond yields had risen after the to the risk of war, Fort Sumter was followed by an large jump in yields.
...
PERIOD_PRE_PANIC <- as.Date(c("1855-3-1", "1855-10-1"))
PERIOD_PANIC <- as.Date(c("1857-9-1", "1858-2-1"))
PERIOD_PRE_ELEC <- as.Date(c("1858-3-1", "1855-10-1"))
PERIOD_POST_ELEC <- as.Date(c("1861-11-7", "1861-4-14"))
PERIOD_WAR_START <- as.Date(c("1861-4-15", "1861-6-1"))

#.prices[.prices[["series"]] == "Kentucky 6s", "group"] <- "North"
prices_north <- 
    (filter(prices_prewar, group == "North")
     %>% group_by(date)
     %>% dplyr::summarise(north_mean = mean(ytm),
                          north_var = var(ytm)))
prices_south <- 
    (filter(prices_prewar, group == "South")
     %>% group_by(date)
...
\caption{Mean spread between the yields of southern and northern state bonds.
  ...
  The border states (Kentucky, Missouri) are excluded.
  }
...
colnames(prices_sumter)

@ 

The market's assessment of the risk of war can also be seen in the spread between the yields of southern and northern state bonds.
...
Figure \ref{fig:north_south_spreads} plots the difference between the mean yields of southern and northern state bonds.

From 1855 to the Panic of 1857, the spread was small (range: \Sexpr{format(round(spread_1855_1857[["lb"]],2))} to \Sexpr{format(round(spread_1855_1857[["ub"]], 2))}.
Although in the Panic of 1857, yields of state bonds spiked, see Figure \ref{fig:yields_all}, the spread between southern and northern bonds increased only slightly; although the financial crisis increased the yields of all state bonds.
From the Panic of 1857 until mid 1860, the average spread was stable but slightly higher than period before the Panic (range: \Sexpr{format(round(spread_1858_1860[["lb"]], 2))} to \Sexpr{format(round(spread_1858_1860[["ub"]], 2))}.
Although the uncertainty over the spread is less than before the Panic of 1857, the interval still includes zero.
...
While the yields and spread between northern and southern bonds indicates changes in the risk leading up to the war, they are not directly interpretable as the probability of war.%
However, the market's beliefs about the probability of war can be inferred from bond prices by treating war as a default event.
...
\footnote{\textcite[115]{WaldenstromFrey2008} propose a similar method, that when the war is observed the probability of war is the proportion of the yield after war to the yield prior to war. 
  That method is not directly derived from a cashflow model and produces higher estimates of the probability of war.
...
To estimate the probability of war, the risk-free interest rate $r$ is the yield of a comparable bond with no-risk of war.
In this case, $r$ will not necessarily be risk-free, and should still include any risks to the bond not due to war, while excluding risk due to war.
One estimate of $r$ the yield of the same bond for an earlier period in which the risk of war is approximately zero.
For each bond considered here, $r$ is the mean yield from \Sexpr{DATEFMT_PEACE_START} to \Sexpr{DATEFMT_PEACE_END}.%
\footnote{
  The Panic of 1857 (August 1857 through January 1858) is excluded from calculations because as discussed earlier in this paper, while the yields increased this it was due to a financial crisis and not war risk.
...
Although there may not have been no risk of war during this period, as discussed, the yields were relatively low in this period, and thus the the risk could not have been that large.
...
Since the American Civil War did occur, $R$ is known --- it is the price observed at the start of the war.
Fo reach bond, the value of $R$ is the average price \Sexpr{DATEFMT_WAR_START} to \Sexpr{DATEFMT_WAR_END}. 
...
\footnote{To control for differences in bond prices over time due to changes in maturity, the $R_{t}$ for each bond is the present value of the bond at time $t$ calculated using the average yield to maturity from the period after the start of the war.}
...
Figure~\ref{fig:prwar} plots the implied probabilities of war for southern and border state and U.S. government bonds for the period after the Panic of 1857 to the Battle of Fort Sumter.
...
@ 
  \caption{Implied probabilities of the initiation of war calculated from U.S. government and southern and border state bonds, \Sexpr{DATEFMT_RANGE_START} to \Sexpr{DATEFMT_RANGE_END}.}
  \label{fig:prwar}
\end{figure}
...
\textcite{Ferguson2006} shows that although World War I was preceeded by a series of crises, European markets did not anticipate the onset of WWI.

Market \textit{ex ante} assessments provide a potentially interesting way for assessing theories of war.
The two rationalist theories of war: commitment problems and private information, have different implications as to what the ex ante probability of war is.
In the stylized, naive version of the commitment problem, given the condition to cause a war, the war will occur with certainty.
In a private information model of 
