...
\begin{align}
  \label{eq:1}
  y_{t} &= y^{*}_{t - 1} + \Delta y^{*}_{t} + \varepsilon_{t} & \text{where $y_{t}$ not missing.} \\
  \label{eq:9}
...
\footnote{
  The location $a$ and scale $b$ of the log-normal distribution are matched to the empirical location and scale parameters of bid-ask price spreads of Sixes form a number of articles in \textit{Bankers' Magazine}.
}
...
The weight assigned to the effect of each battle on each day is based on the number of articles and the pages on which those articles appeared.
Let $w_{t}$ be the news about a battle on day $t$, $n_{t}$ be the number of articles about that battle, and $\mathtt{page}_{i}$ be the page on which article $i$ appeared. 
The news of a battle depends both on articles on that day and the previous day. 
\begin{equation*}
  w_{t} = \frac{1}{2}\sum_{i = 1}^{n_{t}} \frac{1}{\mathtt{page}_{i}} +  \frac{1}{2} \sum_{i = 1}^{n_{t - 1}} \frac{1}{\mathtt{page}_{j}}
\end{equation*}
