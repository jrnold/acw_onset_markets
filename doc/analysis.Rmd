...
```{r header,results = 'hide',echo = FALSE, error = FALSE, cache.extra = tools::md5sum(.RPROFILE)}
...
```{r war_initiation, results='hide', cache.extra = tools::md5sum("filehashdb/prices0")}
...
dat$pre_panic_means_min <- round(pre_panic_means[1, "ytm_mean"], 2)
dat$pre_panic_means_min_name <- pre_panic_means[1, "series"]
dat$pre_panic_means_max <- round(pre_panic_means[nrow(pre_panic_means), "ytm_mean"], 2)
dat$pre_panic_means_max_name <- pre_panic_means[nrow(pre_panic_means), "series"]

```


```{r south_north_spread_data, results = 'hide', cache.extra = tools::md5sum(file.path("filehashdb", "spreads2_summary"))}
...
```{r prob_war_data, cache.extra = tools::md5sum(file.path("filehashdb", c("prwar1", "prwar4_summary")))}
