...
prices_sumter <- 
    local({
        ...
        .data <- mutate(dcast(.data, series ~ date, value.var = "ytm"),
                        ...
                        `\\% Change` = (`Apr 20, 1861` - `Apr 13, 1861`) / `Apr 20, 1861`)
...
local({
    ...
    rownames(x) <- x[["series"]]
    x[["series"]] <- NULL
    colnames(x) <- c("\\parbox{2cm}{Avg. yield \\\\ Feb 1858--\\\\ Nov 1860)}",
                     "\\parbox{2cm}{Avg. yield \\\\ Nov 1860--\\\\ Apr 1861}",
                     "\\parbox{2cm}{\\centering Peace yield}",
                     "\\parbox{2cm}{\\centering Avg. price \\\\ (1861-4-13 \\\\ 1861-5-18)}")
...
Table \ref{fig:sumter} shows yields in the week immediately before and after news of the surrender of Fort Sumter reached the New York market. 
Southern state yields fell between 6.1 (Tennessee) and 7.7 (Virginia) percentage points; or between 41 and 45 percent.
...
These cases suggest in in many cases financial markets do not price in the the risk of war.
To the extent that markets are efficient and incorporating public information, this suggests that given public information, many wars are not anticipated.
