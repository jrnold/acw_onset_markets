...
Figure \ref{fig:north_south_spreads} plots the estimated difference between the mean yields of southern and northern state bonds from that model.

Over the entire period considered here, the spread was positive, meaning that southern state bonds were riskier on average than northern state bonds, although this could be due to a number of factors other.
...
the mean difference between the spread in January 1860 and July 1857 was \Sexpr{round(SPREAD_DIFF_1860_1857_MEAN, 2)}.%
...
The difference between the spread on November 1860 and January 1860 had a mean \Sexpr{round(SPREAD_DIFF_1860_MEAN, 2)}.%
\footnote{The probability the spread is greater than 0 is $\Pr(x > 0) = \Sexpr{round(SPREAD_DIFF_1860_PVAL, 3)}$.}
Although there was increased attention in the market to political issues in the October prior to Lincoln's election, the spread shows little change during that month.
After the election of Lincoln, the spread jumps immediately and then continues to increase until the start of the war.
The spread increased \Sexpr{round(SPREAD_DIFF_ELEC, 2)} percentage points from November 2--23, and another \Sexpr{round(SPREAD_DIFF_POSTELEC, 2)} from November 23 to April 13, 1861, reaching a value of \Sexpr{round(SPREAD_18610413, 2)}.
...
The estimated war risk increased sharply after the Lincoln's victory election of 1860, but it 
